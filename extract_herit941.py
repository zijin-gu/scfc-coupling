...
subjects = np.genfromtxt(herit_dir + 'herit_subject941.txt', dtype='int')
...
def extract_herit_sc(sc_type):
    ...
    mdic = {"C": sc}
    sio.savemat(herit_dir + 'cc400_SC_' + sc_type + '_herit941.mat', mdic)
...
def extract_herit_fc(gamma):
    ...
    for i in range(subjects.shape[0]):
        ...
        np.fill_diagonal(fc_corr1[i], 1)
        np.fill_diagonal(fc_corr2[i], 1)
        np.fill_diagonal(fc_corr3[i], 1)
        np.fill_diagonal(fc_corr4[i], 1)
    ...
    sio.savemat(herit_dir + "cc400_FCcorr_hpf_941subj_session1.mat", mdic)

    mdic = {"C": fc_corr2, "subj": subjects}
    sio.savemat(herit_dir + "cc400_FCcorr_hpf_941subj_session2.mat", mdic)

    mdic = {"C": fc_corr3, "subj": subjects}
    sio.savemat(herit_dir + "cc400_FCcorr_hpf_941subj_session3.mat", mdic)

    mdic = {"C": fc_corr4, "subj": subjects}
    sio.savemat(herit_dir + "cc400_FCcorr_hpf_941subj_session4.mat", mdic)

    mdic = {"C": fc_prec1, "subj": subjects, "gamma": gamma}
    sio.savemat(herit_dir + "cc400_FCprec_hpf_941subj_session1.mat", mdic)

    mdic = {"C": fc_prec2, "subj": subjects, "gamma": gamma}
    sio.savemat(herit_dir + "cc400_FCprec_hpf_941subj_session2.mat", mdic)

    mdic = {"C": fc_prec3, "subj": subjects, "gamma": gamma}
    sio.savemat(herit_dir + "cc400_FCprec_hpf_941subj_session3.mat", mdic)

    mdic = {"C": fc_prec4, "subj": subjects, "gamma": gamma}
    sio.savemat(herit_dir + "cc400_FCprec_hpf_941subj_session4.mat", mdic)


def gen_herit_scfc_couping(sc_type, fc_type, session_id):

    sc = sio.loadmat(herit_dir + 'cc400_SC_' + sc_type + '_herit941.mat')['C']
    fc = sio.loadmat(herit_dir + 'cc400_FC' + fc_type +
                     '_hpf_941subj_session' + str(session_id) + '.mat')['C']
    ...
    sio.savemat(herit_dir + 'regionalcp_hpf_' + fc_type +
                '_herit941_session' + str(session_id) + '.mat', mdic)
...
def standardize(data_type, fc_type=None, session_id=None):
    ...
    if data_type != 'cp':
        ...
        if data_type == 'sc':
            data = sio.loadmat(herit_dir + 'cc400_SC_sift2volnorm_herit941.mat')['C']
        ...
        data = sio.loadmat(herit_dir + 'regionalcp_hpf_' + fc_type +
                           '_herit941_session' + str(session_id) + '.mat')['cp']
